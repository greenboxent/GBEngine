...
export default class ModalBase {
    ...
    _setupResizeRestart() {
        ...
        const onResize = (gameSize) => {
            ...
            if (window.Debug?.core?.enabled) console.log(`[ModalBase:${scene.sys.settings.key}] resize width delta triggers restart-scheduling: ${_lastW} -> ${gameSize.width} (height ${_lastH} -> ${gameSize.height}), active=${scene.scene.isActive()}, sleeping=${scene.scene.isSleeping()}`);
            ...
            if (scene.scene.isActive() && !scene.scene.isSleeping()) {
                ...
                _resizeTimer = setTimeout(() => {
                    ...
                    if (window.Debug?.core?.enabled) console.log(`[ModalBase:${scene.sys.settings.key}] restarting scene now (resize)`);
